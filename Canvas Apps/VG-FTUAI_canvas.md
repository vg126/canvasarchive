...
<body class="bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors">
    <div class="container mx-auto px-4 py-6 max-w-7xl">
        ...
        <div class="flex flex-col items-center gap-4 mb-8">
            <div class="flex items-center gap-2">
                <input type="checkbox" id="include-websearch" class="w-4 h-4">
                <label for="include-websearch" class="text-sm font-medium">Include Web Search in main generation</label>
            </div>
    ...
    <script>
        ...
        const categories = ['vg', 'good', 'gemini', 'exotica', 'websearch', 'websearch2'];
        ...
        async function generateResponseForCategory(category, isWebSearchOnly = false) {
            return new Promise(async (resolve, reject) => {
                ...
                logToDebug(`📄 Prompts - main: ${mainPrompt.length} chars, system: ${systemPrompt.length} chars, budget: ${thinkingBudget}`);
                ...
                try {
                    ...
                    // Add web search context if available and this is not a web search category
                    if (!isWebSearchOnly && Object.keys(webSearchResults).length > 0) {
                        fullPrompt += '[WEB SEARCH CONTEXT]\n';
                        Object.entries(webSearchResults).forEach(([category, content]) => {
                            const categoryName = category === 'websearch' ? 'Web Search' : 'Web Search 2';
                            fullPrompt += `${categoryName}: ${content}\n\n`;
                        });
                        fullPrompt += '[END WEB SEARCH CONTEXT]\n\n';
                        logToDebug(`📄 Added web search context to ${category}: ${Object.keys(webSearchResults).length} results`);
                    }
                    ...
                    window.Poe.registerHandler(handlerName, (result) => {
                        ...
                        if (msg.status === 'error') {
                            ...
                            try {
                                ...
                                logToDebug(`⚠️ Markdown parse error for ${category}: ${e.message}`);
        ...
        // Main generate button - with WEB SEARCH FIRST logic
        document.getElementById('generate-btn').addEventListener('click', async () => {
            logToDebug('🔥 GENERATE BUTTON CLICKED!');
            
            const checkedCategories = Array.from(document.querySelectorAll('.model-checkbox:checked'))
                .map(cb => cb.dataset.category);
            
            logToDebug(`📋 Found ${checkedCategories.length} checked categories: ${checkedCategories.join(', ')}`);
            
            if (checkedCategories.length === 0) {
                logToDebug('❌ No models selected');
                showNotification('❌ Please select at least one model');
                return;
            }
            
            const mainPrompt = document.getElementById('main-prompt').value;
            logToDebug(`📝 Main prompt: "${mainPrompt}" (length: ${mainPrompt.length})`);
            
            if (!mainPrompt.trim()) {
                logToDebug('❌ Empty prompt');
                showNotification('❌ Please enter a main prompt');
                return;
            }
            ...
            // Separate web search categories from others
            const webSearchCategories = checkedCategories.filter(cat => cat === 'websearch' || cat === 'websearch2');
            const otherCategories = checkedCategories.filter(cat => cat !== 'websearch' && cat !== 'websearch2');
            
            logToDebug(`🔍 Web search categories: ${webSearchCategories.join(', ')}`);
            logToDebug(`📝 Other categories: ${otherCategories.join(', ')}`);
            
            // PHASE 1: Run web search first if any web search categories are selected
            if (webSearchCategories.length > 0) {
                logToDebug('🔍 PHASE 1: Running web search first...');
                showNotification('🔍 Running web search first...');
                
                // Run all web search categories and wait for completion
                const webSearchPromises = webSearchCategories.map(async (category) => {
                ...
                logToDebug('🔍 All web search categories completed');
                ...
                webSearchCategories.forEach(category => {
                    ...
                    if (responseEl) {
                        ...
                        if (content && !content.includes('Select a model') && !content.includes('Generating response')) {
                            ...
                            webSearchResults[category] = '[No results or error]';
                            logToDebug(`⚠️ No valid web search result for ${category}`);
                        }
                    }
                });
            }
            
            // PHASE 2: Run other categories with web search context
            if (otherCategories.length > 0) {
                if (webSearchCategories.length > 0) {
                    logToDebug('📝 PHASE 2: Running other models with web search context...');
                    showNotification('📝 Now generating other responses with web search context...');
                } else {
                    logToDebug('🚀 Running all categories (no web search)');
                }
                
                for (let i = 0; i < otherCategories.length; i++) {
                    const category = otherCategories[i];
                    logToDebug(`🔄 Processing category ${i + 1}/${otherCategories.length}: ${category}`);
                    
                    try {
                        await generateResponseForCategory(category, false);
                        logToDebug(`✅ Finished processing ${category}`);
                    } catch (error) {
                        logToDebug(`❌ Error processing ${category}: ${error.message}`);
                    }
                }
            }
            
            logToDebug('🏁 All categories processed');
            if (webSearchCategories.length > 0 && otherCategories.length > 0) {
                showNotification('✅ All responses generated with web search context!');
            } else {
                showNotification('✅ All responses generated!');
            }
        });
...
## Metadata
- **Extracted**: 2025-07-27T19:15:30.157Z
- **Source**: https://poe.com/edit_bot?bot=VG-FTUAI
- **Bot Type**: Canvas App
- **Code Length**: 48977 characters
