...
<body class="bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors">
    <div class="container mx-auto px-4 py-6 max-w-7xl">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            ...
            <!-- Fast and Furious -->
            <div class="dropdown-container">
                <label class="block text-sm font-medium mb-2">Fast and Furious</label>
                <div class="flex items-center gap-2">
            ...
            <!-- Web Search -->
            <div class="dropdown-container">
                <label class="block text-sm font-medium mb-2">Goku</label>
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="enable-raditz" class="model-checkbox" data-category="raditz">
                    <select id="model-raditz" class="flex-1 p-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-base">
                        <option value="">Select Model</option>
                ...
                <div id="cost-raditz" class="cost-badge text-white text-xs px-2 py-1 rounded-full mt-1 hidden"></div>
            </div>

            <!-- Web Search 2 -->
            <div class="dropdown-container">
                <label class="block text-sm font-medium mb-2">Vegeta</label>
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="enable-nappa" class="model-checkbox" data-category="nappa">
                    <select id="model-nappa" class="flex-1 p-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-base">
                        <option value="">Select Model</option>
                ...
                <div id="cost-nappa" class="cost-badge text-white text-xs px-2 py-1 rounded-full mt-1 hidden"></div>
            </div>
        </div>
        ...
        <div class="flex flex-col items-center gap-4 mb-8">
            <div class="flex justify-center gap-4 flex-wrap">
                <button id="websearch1-btn" class="px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-semibold rounded-lg hover:from-blue-700 hover:to-cyan-700 transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
                    Web Search 1 (Goku)
                </button>
                <button id="websearch2-btn" class="px-6 py-3 bg-gradient-to-r from-green-600 to-teal-600 text-white font-semibold rounded-lg hover:from-green-700 hover:to-teal-700 transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed">
                    Web Search 2 (Vegeta)
                </button>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 mb-6">
            ...
            <div class="response-container" data-category="good">
                <div class="flex justify-between items-center mb-3">
                    <h3 class="font-semibold text-lg">Fast and Furious</h3>
            ...
            <!-- Web Search Response -->
            <div class="response-container" data-category="raditz">
                <div class="flex justify-between items-center mb-3">
                    <h3 class="font-semibold text-lg">Goku</h3>
                    <button class="copy-individual px-3 py-1 bg-gray-500 text-white rounded text-sm hover:bg-gray-600 transition-colors" data-category="raditz">
                        Copy
                    </button>
                </div>
                <div id="response-raditz" class="response-box p-4 rounded-lg bg-gray-50 dark:bg-gray-800 overflow-auto">
                    <p class="text-gray-500 dark:text-gray-400 italic">Select a model and generate to see response</p>
                </div>
            </div>

            <!-- Web Search 2 Response -->
            <div class="response-container" data-category="nappa">
                <div class="flex justify-between items-center mb-3">
                    <h3 class="font-semibold text-lg">Vegeta</h3>
                    <button class="copy-individual px-3 py-1 bg-gray-500 text-white rounded text-sm hover:bg-gray-600 transition-colors" data-category="nappa">
                        Copy
                    </button>
                </div>
                <div id="response-nappa" class="response-box p-4 rounded-lg bg-gray-50 dark:bg-gray-800 overflow-auto">
                    <p class="text-gray-500 dark:text-gray-400 italic">Select a model and generate to see response</p>
                </div>
            </div>
        </div>
...
- Draw insights and conclusions confidently
        ...
        const categories = ['vg', 'good', 'exotica', 'raditz', 'nappa'];
        ...
        async function generateResponseForCategory(category, isWebSearchOnly = false) {
            return new Promise(async (resolve, reject) => {
                ...
                try {
                        ...
                        if (Object.keys(webSearchResults).length > 0) {
                            ...
                            Object.entries(webSearchResults).forEach(([searchCategory, content]) => {
                                try {
                                    ...
                                    const categoryName = searchCategory === 'search1' ? 'Goku' : 'Vegeta';
                    ...
                    const handlerName = 'h' + crypto.randomUUID().replace(/-/g,'');
        ...
        // Web Search 1 Button - Goku only
        document.getElementById('websearch1-btn').addEventListener('click', () => {
            logToDebug('WEB SEARCH 1 BUTTON CLICKED!');
            ...
            if (checkbox.checked && select.value) {
                ...
                showNotification('Running Web Search 1 (Goku)...');
            } else {
                showNotification('Please select and enable Web Search 1 (Goku)');
            }
        });

        // Web Search 2 Button - Vegeta only
        document.getElementById('websearch2-btn').addEventListener('click', () => {
            logToDebug('WEB SEARCH 2 BUTTON CLICKED!');
            ...
            if (checkbox.checked && select.value) {
                ...
                showNotification('Running Web Search 2 (Vegeta)...');
            } else {
                showNotification('Please select and enable Web Search 2 (Vegeta)');
            }
        });
...
## Metadata
- **Extracted**: 2025-07-27T19:15:34.822Z
- **Source**: https://poe.com/edit_bot?bot=VG-GOGETA
- **Bot Type**: Canvas App
- **Code Length**: 48067 characters
